...
INTERVIEW_SYSTEM_INSTRUCTION = (
    ...
    "- ЗАЩИТА ОТ СМЕНЫ ТЕМЫ (КРИТИЧЕСКИ ВАЖНО): Если пациент меняет тему, задает вопросы не про свое здоровье (о погоде, рецептах, других темах) или просит выполнить стороннюю задачу, ты ОБЯЗАН мягко, но твердо отказать. Извинись, напомни, что ты специализированный кардио-ассистент и твоя цель — подготовить данные для врача, после чего СРАЗУ задай следующий невыясненный вопрос из анкеты.\n"
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner("⏳ Структурирование данных и очистка от визуального мусора..."):
            EXTRACTION_SYSTEM_INSTRUCTION = (
                ...
                "и извлечь из неё сухие медицинские факты, полностью игнорируя любые бытовые темы, эмоции, рассуждения "
                "о погоде, хобби и прочий нерелевантный текст.\n\n"
                ...
                "8. История болезней (кардиологическая и эндокринная):\n"
                "9. Наследственность:\n"
                "10. СОПУТСТВУЮЩИЕ ФАКТОРЫ (любые другие названные пациентом симптомы, боли, текущие заболевания или принимаемые лекарства):\n\n"
                "КРИТИЧЕСКОЕ ПРАВИЛО: Верни только этот нумерованный список. Никаких вводных слов и выводов."
            )

            # Извлекаем "чистую" медицинскую карту
            clean_anamnesis = call_yandex_gpt([{"role": "user", "content": full_dialogue_history}],
                                              EXTRACTION_SYSTEM_INSTRUCTION, temperature=0.1)

            # Показываем врачу/пациенту, как система поняла их слова (очень полезно для отладки)
        ...
        with st.spinner("🔍 Идет поиск по клиническим рекомендациям в базе данных Qdrant..."):
            # ПЕРЕДАЕМ В ОПТИМИЗАТОР ЗАПРОСОВ ТОЛЬКО ЧИСТЫЙ АНАМНЕЗ, БЕЗ СЫРОГО ДИАЛОГА!
